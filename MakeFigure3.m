...
% Plot correlations
subplot(2,3,3)
errorbar(distbins,SCcorrSimMean,SCcorrSimErr,'k','Linewidth',1.5)
hold on
axis([-.02 .47 -.2e-3 1.5e-3])
...
%%%%%%%%%%%%%%%%%%%%
% Compute and plot theoretical corrs
%%%%%%%%%%%%%%%%%%%%
 
qe=Ne/N;
qi=Ni/N;
qF=NF/N;
jei=-abs(jei); % These are wrong sign in mat file
jii=-abs(jii);
W0=[qe*pee0*jee qi*pei0*jei; qe*pie0*jie qi*pii0*jii];
Fbar=[jeF*peF0; jiF*piF0]*qF*rF;
rates=-inv(W0)*Fbar;
re=rates(1);ri=rates(2);
%re=reSim/1000;ri=riSim/1000;
R=[sqrt(re) 0; 0 sqrt(ri)];
CFF0=[jeF^2*peF0^2 jeF*peF0*jiF*piF0; jiF*piF0*jeF*peF0 jiF^2*piF0^2]*qF*rF;
sigmaSS=sqrt(2*sigmaeF^2-2*sigmaee^2);
 
rho=@(d)((1/N)*inv(R)*inv(W0)*CFF0*inv(W0')*inv(R)*exp(-d^2./(2*sigmaSS^2))/(2*pi*sigmaSS^2)+(1/N)*[1/qe 0; 0 1/qi]);
rhoee=@(d)(jei*jiF*pei0*piF0-jeF*jii*peF0*pii0)^2*qF*rF*exp(-d.^2/(2*sigmaSS^2))/(N*(jei*jie*pei0*pie0-jee*jii*pee0*pii0)^2*qe^2*re*2*pi*sigmaSS^2);
TC=rhoee(distances);
 
% Compute mean correlation and stderr of correlation
% over each distance bin
SCcorrTheory=zeros(size(distbins));
for j=1:numel(distbins)
    SCcorrTheory(j)=mean(TC(I==j));
end
 
subplot(2,3,3)
plot(distbins,SCcorrTheory,'r--','LineWidth',2)
...
% Initialize storage for time-resolved correlations
nTimeBins = 100;
winsize = 250;

% Define the edges of time bins
timeBins = 0:winsize:T;
timeCorrs = zeros(nTimeBins, 1);

% Loop through time bins and compute correlations
for t = 1:nTimeBins
    % Select spikes within the current time bin
    binStart = ceil((timeBins(t) - Tburn) / winsize);
    binEnd = ceil((timeBins(t + 1) - Tburn) / winsize) - 1;

    % Check if the bins are valid
    if binStart >= 1 && binEnd <= size(counts, 1) && binStart <= binEnd
        binCounts = counts(binStart:binEnd, :);

        % Compute correlations for this time bin
        C_bin = corrcoef(binCounts);

        % Extract lower-triangle correlations (excluding diagonal)
        [II, JJ] = meshgrid(1:size(C_bin, 1), 1:size(C_bin, 1));
        lowerTriangle = (II > JJ);
        corrValues = C_bin(lowerTriangle);

        % Store mean correlation for this time bin
        timeCorrs(t) = mean(corrValues, 'omitnan');
    else
        % If no valid bins, set correlation to NaN
        timeCorrs(t) = NaN;
    end
end
...
figure;
plot(timeBins(1:end-1) + winsize / 2, timeCorrs, 'k', 'LineWidth', 1.5);
xlabel('Time (ms)');
ylabel('Mean Correlation');
title('Spike Correlations Over Time');
grid on;




save('correlation_data2.mat', 'distbins', 'SCcorrSimMean', 'SCcorrSimErr', 'SCcorrTheory', 'bd', 'mFRCovs', 'mFFCovs', 'mRRCovs', 'mTotalCovs','SCorree');
