...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    int printfperiod, kk, Ntref[2], Ni, Ni1, Ne2, Ni2, Kee, Kie, Kei, Kii, Kex, Kix, Ne, Ne1, j, j1, j2, k, i, N, Nt, m1, m2, maxns, ns, flagei, Nrecord, NrecordI, jj, nskiprecord, flag, tempflag, Nsx, Nx1, Nx;
    double *input_e, *input_i, dt, *s, *v, *v0, *JnextE, *JnextI, *JnextX, *alphax, *alphae, *alphai, tausyne, tausyni, *alphaxr, *alphaer, *alphair, *vr, *alphaeri, *alphairi, *alphaxri, *sx, Jex, Jix, betaex, betaix, tausynx;
    double Jee, Jei, Jie, Jii, betaee, betaei, betaie, betaii, T, *Irecord, *IrecordI, *C, *Vleak, *DeltaT, *VT, *tref, *gl, *Vth, *Vre, *Vlb, xloc, yloc;
    ...
    IrecordI = mxGetPr(prhs[39]);
    NrecordI = mxGetN(prhs[39]);
    m2 = mxGetM(prhs[39]);
    ...
    input_e = mxGetPr(plhs[5]);

    plhs[6] = mxCreateDoubleMatrix(1, 1, mxREAL);
    input_i = mxGetPr(plhs[6]);

    plhs[7] = mxCreateDoubleMatrix(NrecordI, Nt, mxREAL);
    alphaeri = mxGetPr(plhs[7]);

    plhs[8] = mxCreateDoubleMatrix(NrecordI, Nt, mxREAL);
    alphairi = mxGetPr(plhs[8]);

    plhs[9] = mxCreateDoubleMatrix(NrecordI, Nt, mxREAL);
    alphaxri = mxGetPr(plhs[9]);
    ...
    long long int count_input_e = 0;
    long long int count_input_i = 0;
    ...
    /* Record input currents and membrane potentials at first time bin for inhibitory */
    for (jj = 0; jj < NrecordI; jj++)
    {
        /* Find index into local variables */
        j1 = (int)round(IrecordI[2 * jj] - 1);
        j2 = (int)round(IrecordI[2 * jj + 1] - 1);

        /* Convert to 1D index, j */
        if (j1 >= Ne1 && j2 >= Ne1) // Ensure both indices are in the inhibitory range
        {
            j = (j1 - Ne1) + (j2 - Ne1) * Ni1 + Ne;
        }
        else
        {
            mexErrMsgTxt("Indices in IrecordI must have both terms >=Ne1 (inhibitory neuron indices).");
        }

        if (j >= N || j < 0)
        {
            mexErrMsgTxt("IrecordI contains out of bounds indices.");
        }

        /* Store currents and membrane potentials at first time bin */
        alphaeri[jj + NrecordI * 0] = alphae[j];
        alphairi[jj + NrecordI * 0] = alphai[j];
        alphaxri[jj + NrecordI * 0] = alphax[j];
    }
    ...
    for (i = 1; i < Nt && ns < maxns; i++)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            double noise = 0.0; // (double)rand() / RAND_MAX * 4.0f - 2.0f;
            ...
            { /* If cell is excitatory */
                ...
                if (refstate[j] <= 0)
                {
                    v[j] += fmax((alphae[j] + alphai[j] + alphax[j] + noise) * dt / C[0], Vlb[0] - v[j]);
                    // v[j] += fmax((alphae[j] + alphai[j] + alphax[j] - gl[0] * (v[j] - Vleak[0])) * dt / C[0], Vlb[0] - v[j]);
                    *input_e = alphax[j] - gl[0] * (v[j] - Vleak[0]);
                    count_input_e++;
                    // v[j] += fmax((alphax[j] - gl[0] * (v[j] - Vleak[0])) * dt / C[0], Vlb[0] - v[j]);
                }
            ...
            { /* If cell is inhibitory */
                ...
                if (refstate[j] <= 0)
                {
                    *input_i += alphax[j] - gl[1] * (v[j] - Vleak[1]);
                    count_input_i++;
                    v[j] += fmax((alphae[j] + alphai[j] + alphax[j] + noise) * dt / C[1], Vlb[1] - v[j]);
                    // v[j] += fmax((alphae[j] + alphai[j] + alphax[j] - gl[1] * (v[j] - Vleak[1])) * dt / C[1], Vlb[1] - v[j]);
                    //  v[j] += fmax((alphax[j] - gl[1] * (v[j] - Vleak[1])) * dt / C[1], Vlb[1] - v[j]);
                }
                else
                {
                    if (refstate[j] > 1)
        ...
        for (jj = 0; jj < NrecordI; jj++)
        {
            ...
            /* Store inhibitory currents and membrane potentials */
            alphaeri[jj + NrecordI * i] = alphae[j];
            alphairi[jj + NrecordI * i] = alphai[j];
            alphaxri[jj + NrecordI * i] = alphax[j];
        }
    ...
    plhs[7] = mxCreateDoubleScalar((double)count_input_e);
    plhs[8] = mxCreateDoubleScalar((double)count_input_i);
