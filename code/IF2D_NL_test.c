...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    mxArray *temp1, *temp2, *temp0, *temp3, *temp4, *temp5, *temp6, *temp7, *temp8, *temp9, *temp10, *temp11, *temp12, *temp13, *temp14, *temp15, *temp16, *temp17, *temp18, *temp19, *temp20;
    ...
    double applySigmoid(double, int);
    ...
    plhs[11] = mxCreateDoubleMatrix((Nrecord * D), Nt, mxREAL);
    ...
    *input_count = 0.0;
    ...
    for (jj = 0; jj < Nrecord; jj++)
    {
        ...
        for (int d = 0; d < D; d++)
        {
            alphaer1[jj * D + Nrecord * 0 + d] = alphae1[(j * D) + d];
        }
    }
    ...
    for (i = 1; i < Nt && ns < maxns; i++)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            for (int d = 0; d < D; d++)
            {
                alphae1[(j * D) + d] -= alphae1[(j * D) + d] * (dt / tausyne);
            }
            ...
            *input_count += 1.0;
            ...
            { /* If cell is excitatory */
                ...
                double average_input_per_dendrite = total_input_current / D;

                if (refstate[j] <= 0)
                    v[j] += fmax(centered_sigmoid * dt / C[0], Vlb[0] - v[j]);
                else
                {
                    if (refstate[j] > 1)
                        v[j] = Vth[0];
                ...
                if (v[j] >= Vth[0] && refstate[j] <= 0 && ns < maxns)
                {
                    ...
                    for (k = 0; k < Kee; k++)
                    {
                        ...
                        JnextE1[postsynaptic_cell + closest_dendrite] += Jee;
                        // if (closest_dendrite == 0)
                        // {
                        //     JnextE1[postsynaptic_cell] += Jee;
                        // }
                        if (closest_dendrite == 1)
                        {
                    ...
                    for (k = 0; k < Kie; k++)
                    {
                        ...
                        JnextE1[postsynaptic_cell + closest_dendrite] += Jie;
                        // if (closest_dendrite == 0)
                        // {
            ...
            { /* If cell is inhibitory */
                ...
                centered_sigmoid = input_sum;
                if (refstate[j] <= 0)
                    v[j] += fmax(centered_sigmoid * dt / C[1], Vlb[1] - v[j]);
                else
                {
                    if (refstate[j] > 1)
                        v[j] = Vth[1];
        ...
        for (jj = 0; jj < Nrecord; jj++)
        {
            ...
            for (int d = 0; d < D; d++)
            {
                alphaer1[jj * D + Nrecord * i + d] = alphae1[(j * D) + d];
            }
        }
        ...
        for (j = 0; j < N; j++)
        {
            ...
            for (int d = 0; d < D; d++)
            {
                ...
                alphae1[(j * D) + d] += JnextE1[(j * D) + d] / tausyne;
                JnextE1[(j * D) + d] = 0;
            }
...
double applySigmoid(double input_sum, int dendrite_type)
{
    double lambda = 1.0; // Default lambda
    double a = 0.0;      // Center point of sigmoid

    // Apply different lambda values based on the dendrite type
    switch (dendrite_type)
    {
    case 0:
        lambda = 1.0; // Excitatory dendrite
        break;
    case 1:
        lambda = 0.5; // Inhibitory dendrite (slower response)
        break;
    case 2:
        lambda = 2.0; // Fast-response dendrite
        break;
    default:
        lambda = 1.0; // Default lambda for unknown types
        break;
    }
