...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double NL_slope, NL_center, NL_height, dt, *s, *v, *v0, *JnextE, *JnextI, *JnextX, *alphax, *alphae, *alphai, tausyne, tausyni, *alphaxr, *alphaer, *alphair, *vr, *sx, Jex, Jix, betaex, betaix, tausynx, *total_ffwd_input, *total_recurrent_ex_input, *total_recurrent_inh_input, *total_input, *input_count, *input, *JnextE1, *JnextI1, *JnextX1, *alphax1, *alphae1, *alphai1, *alphaxr1, *alphaer1, *alphair1, *JnextE2, *JnextI2, *JnextX2, *alphax2, *alphae2, *alphai2, *alphaxr2, *alphaer2, *alphair2, *JnextE3, *JnextI3, *JnextX3, *alphax3, *alphae3, *alphai3, *alphaxr3, *alphaer3, *alphair3, *JnextE4, *JnextI4, *JnextX4, *alphax4, *alphae4, *alphai4, *alphaxr4, *alphaer4, *alphair4, *input_NL, *input_dendrite_1_before, *input_dendrite_1_after, *input_NL_after;
    ...
    NL_center = mxGetScalar(prhs[40]);
    NL_height = mxGetScalar(prhs[41]);
    NL_slope = mxGetScalar(prhs[42]);

    /******
     * Finished importing variables.
     *******/

    double lambda = NL_slope; // 0.5; // 0.7;
    ...
    *input_count = 0.0;
    ...
    //     for (int d = 0; d < D; d++)
    //     {
    //         dendriteArray[i][d] = 0.0; // Set initial values
    //     }
    // }
    ...
    for (i = 1; i < Nt && ns < maxns; i++)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            *input_count += 1.0;
            ...
            double average_input_per_dendrite = NL_center; // 0.0; //-1.125; // 2.5; //-2.5; //-1.4; //-0.9; //-0.5; // 0.23223439321163172; // 0.2569022007446712; // broader inhibition  //0.23223439321163172; // balanced NW inhib = exit
                                                           //    - (gl[0] * (v[j] - Vleak[0]) / 4)

            double noise = (double)rand() / RAND_MAX * 4.0f - 2.0f;
            noise = 0.0; // noise / 4.0;

            double total_input_dendrite_1 = alphae1[j] + alphai1[j] + alphax1[j] + noise;
            double total_input_dendrite_2 = alphae2[j] + alphai2[j] + alphax2[j] + noise;
            double total_input_dendrite_3 = alphae3[j] + alphai3[j] + alphax3[j] + noise;
            double total_input_dendrite_4 = alphae4[j] + alphai4[j] + alphax4[j] + noise;

            input_dendrite_1_before[j] = total_input_dendrite_1;

            double s_at_zero = applySigmoid(0.0, lambda, average_input_per_dendrite * 2);                                                           // s(0)
            double nonlinearity_1 = (NL_height * (2 * applySigmoid(total_input_dendrite_1, lambda, average_input_per_dendrite)) - (2 * s_at_zero)); // 4 * 1.95 *  // 2.3 std or 2*2.3
            double nonlinearity_2 = (NL_height * (2 * applySigmoid(total_input_dendrite_2, lambda, average_input_per_dendrite)) - (2 * s_at_zero));
            double nonlinearity_3 = (NL_height * (2 * applySigmoid(total_input_dendrite_3, lambda, average_input_per_dendrite)) - (2 * s_at_zero));
            double nonlinearity_4 = (NL_height * (2 * applySigmoid(total_input_dendrite_4, lambda, average_input_per_dendrite)) - (2 * s_at_zero));
            // alphae1[j] = nonlinearity_1;
            // alphae2[j] = nonlinearity_2;
            // alphae3[j] = nonlinearity_3;
            // alphae4[j] = nonlinearity_4;
            input_dendrite_1_after[j] = nonlinearity_1;

            double nonlinearity = nonlinearity_1 + nonlinearity_2 + nonlinearity_3 + nonlinearity_4;

            centered_sigmoid = nonlinearity_1;
            input_d1 = total_input_dendrite_1;
            double centered_sigmoid2 = nonlinearity;

            if (j < Ne)
            { /* If cell is excitatory */

                // double input_sum = alphae[j] + alphai[j] + alphax[j] - gl[0] * (v[j] - Vleak[0]);
                // // print input sum in c
                // // mexPrintf("input sum ex: %f\n", input_sum);

                // // Apply sigmoid to input sum

                // double sigmoid_input = 1.0 / (1.0 + exp(-lambda * (input_sum - a))); // s(x)
                // double s_at_zero = 1.0 / (1.0 + exp(-lambda * (0.0 - a)));           // s(0)
                // double centered_sigmoid;
                // // Shifted sigmoid
                // if (dendrite_type[j] == 0)
                // {
                //     centered_sigmoid = sigmoid_input - s_at_zero;
                // }
                // else
                // {
                //     centered_sigmoid = sigmoid_input - s_at_zero;
                ...
                if (v[j] >= Vth[0] && refstate[j] <= 0 && ns < maxns)
                {
                    ...
                    for (k = 0; k < Kee; k++)
                    {
                        ...
                        // dendriteArray[postsynaptic_cell][closest_dendrite] += Jee;

                        /* Add print statement */
                        // // mexPrintf("Pre-synaptic cell (exc): %d, Post-synaptic cell (exc): %d\n", j, postsynaptic_cell);
                        // //  get positions on the space
                        // /* Compute positions in space for pre- and post-synaptic cells */
                        // int post_x = postsynaptic_cell / Ne1; /* Post-synaptic row */
                        // int post_y = postsynaptic_cell % Ne1; /* Post-synaptic column */
                        // // mexPrintf("Pre-synaptic cell (exc): (%d, %d), Post-synaptic cell (exc): (%d, %d)\n", pre_x, pre_y, post_x, post_y);

                        // /* Find the closest dendrite */
                        // int dendrite = closest_dendrite(pre_x, pre_y, post_x, post_y, Ne1 - 1, Ne1 - 1);
                        // mexPrintf("closest dendrite: %d\n", dendrite);
                    }
                    for (k = 0; k < Kie; k++)
                    {
                        ...
                        // /* Add print statement */
                        // // mexPrintf("Pre-synaptic cell (exc): %d, Post-synaptic cell (inh): %d\n", j, postsynaptic_cell);
                        // //  get positions on the space
                        // int post_x = postsynaptic_cell / Ni1; /* Post-synaptic row */
                        // int post_y = postsynaptic_cell % Ni1; /* Post-synaptic column */
                        // // mexPrintf("Pre-synaptic cell (exc): (%d, %d), Post-synaptic cell (inh): (%d, %d)\n", pre_x, pre_y, post_x, post_y);
                        // /* Find the closest dendrite */
                        // int dendrite = closest_dendrite(pre_x, pre_y, post_x, post_y, Ne1 - 1, Ne1 - 1);
                        // // mexPrintf("closest dendrite: %d\n", dendrite);
                    }
            ...
            { /* If cell is inhibitory */
                // double input_sum = alphae[j] + alphai[j] + alphax[j] - gl[1] * (v[j] - Vleak[1]);
                // // print input sum in c
                // // mexPrintf("input sum ex: %f\n", input_sum);
                ...
                // // Shifted sigmoid
                // double centered_sigmoid = sigmoid_input - s_at_zero;
                // centered_sigmoid = input_sum;
                ...
                if (v[j] >= Vth[1] && refstate[j] <= 0 && ns < maxns)
                {
                    ...
                    for (k = 0; k < Kei; k++)
                    {
                        ...
                        // /* Add print statement */
                        // // mexPrintf("Pre-synaptic cell (inh): %d, Post-synaptic cell (exc): %d\n", j, postsynaptic_cell);
                        // // mexPrintf("Pre-synaptic cell (inh): (%d, %d), Post-synaptic cell (exc): (%d, %d)\n", pre_x, pre_y, post_x, post_y);
                        // /* Find the closest dendrite */
                        // int dendrite = closest_dendrite(pre_x, pre_y, post_x, post_y, Ni1 - 1, Ni1 - 1);
                        // dendrite_type[j] = dendrite;
                        // mexPrintf("closest dendrite: %d\n", dendrite);
                    }
                    for (k = 0; k < Kii; k++)
                    {
                        ...
                        // /* Add print statement */
                        // // mexPrintf("Pre-synaptic cell (inh): %d, Post-synaptic cell (inh): %d\n", j, postsynaptic_cell);
                        // // mexPrintf("Pre-synaptic cell (inh): (%d, %d), Post-synaptic cell (inh): (%d, %d)\n", pre_x, pre_y, post_x, post_y);
                        // /* Find the closest dendrite */
                        // int dendrite = closest_dendrite(pre_x, pre_y, post_x, post_y, Ni1 - 1, Ni1 - 1);
                        // dendrite_type[j] = dendrite;
                        // mexPrintf("closest dendrite: %d\n", dendrite);
    ...
    // testprint dendrite array
    // for (int i = 0; i < N; i++)
    // {
    //     for (int d = 0; d < D; d++)
    //     {
    //         mexPrintf("dendriteArray[%d][%d]: %f\n", i, d, dendriteArray[i][d]);
    //     }
    // }
    ...
    // for (int i = 0; i < N; i++)
    // {
    //     mxFree(dendriteArray[i]);
    // }
    // mxFree(dendriteArray);
}
