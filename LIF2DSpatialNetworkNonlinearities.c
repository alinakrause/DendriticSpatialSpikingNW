    ...
    for (i = 1; i < Nt && ns < maxns; i++)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            *input_count += 1.0;
            ...
            double total_input_dendrite_1 = alphae1[j] + alphai1[j] + alphax1[j] + noise - leak_current_per_dendrite; // + leak_current;
            double total_input_dendrite_2 = alphae2[j] + alphai2[j] + alphax2[j] + noise - leak_current_per_dendrite; // + leak_current;
            double total_input_dendrite_3 = alphae3[j] + alphai3[j] + alphax3[j] + noise - leak_current_per_dendrite; // + leak_current;
            double total_input_dendrite_4 = alphae4[j] + alphai4[j] + alphax4[j] + noise - leak_current_per_dendrite; // + leak_current;

            input_dendrite_1_before[j] = total_input_dendrite_1;

            double s_at_zero = applySigmoid(0.0, lambda, 0.0);                                                                                                  // s(0)
            double nonlinearity_1 = (NL_height * (2 * applySigmoid(total_input_dendrite_1, lambda, average_input_per_dendrite) - (2 * s_at_zero))) + NL_center; // 4 * 1.95 *  // 2.3 std or 2*2.3
            double nonlinearity_2 = (NL_height * (2 * applySigmoid(total_input_dendrite_2, lambda, average_input_per_dendrite) - (2 * s_at_zero))) + NL_center;
            double nonlinearity_3 = (NL_height * (2 * applySigmoid(total_input_dendrite_3, lambda, average_input_per_dendrite) - (2 * s_at_zero))) + NL_center;
            double nonlinearity_4 = (NL_height * (2 * applySigmoid(total_input_dendrite_4, lambda, average_input_per_dendrite) - (2 * s_at_zero))) + NL_center;
            // double new_slope = 0.0095;
            // nonlinearity_1 = 450 * (applySigmoid(total_input_dendrite_1, new_slope, 0.0) - 0.5f);
            // nonlinearity_2 = 450 * (applySigmoid(total_input_dendrite_2, new_slope, 0.0) - 0.5f);
            // nonlinearity_3 = 450 * (applySigmoid(total_input_dendrite_3, new_slope, 0.0) - 0.5f);
            // nonlinearity_4 = 450 * (applySigmoid(total_input_dendrite_4, new_slope, 0.0) - 0.5f);
            ...
            { /* If cell is excitatory */
                ...
                if (refstate[j] <= 0)
                    v[j] += fmax(nonlinearity * dt / C[0], Vlb[0] - v[j]);
                else
                {
                    if (refstate[j] > 1)
                        v[j] = Vth[0];
            ...
            { /* If cell is inhibitory */
                ...
                if (refstate[j] <= 0)
                    v[j] += fmax(nonlinearity * dt / C[1], Vlb[1] - v[j]);
                else
                {
                    if (refstate[j] > 1)
                        v[j] = Vth[1];
