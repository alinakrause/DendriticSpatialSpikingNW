...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    *input_count = 0.0;
    ...
    for (i = 1; i < Nt && ns < maxns; i++)
    {
        ...
        for (j = 0; j < N; j++)
        {
            ...
            *input_count += 1.0;
            ...
            double average_input_per_dendrite = NL_center;

            double noise = (double)rand() / RAND_MAX * 4.0f - 2.0f;
            noise = 0.0;
            double leak = (gl[0] * (v[j] - Vleak[0])) / 4.0f;

            // dendritic input currents
            double total_input_dendrite_1 = alphae1[j] + alphai1[j] + alphax1[j] + noise - leak;
            double total_input_dendrite_2 = alphae2[j] + alphai2[j] + alphax2[j] + noise - leak;
            double total_input_dendrite_3 = alphae3[j] + alphai3[j] + alphax3[j] + noise - leak;
            double total_input_dendrite_4 = alphae4[j] + alphai4[j] + alphax4[j] + noise - leak;

            input_dendrite_1_before[j] = total_input_dendrite_1;

            // nonlinear integration
            double s_at_zero = applySigmoid(0.0, lambda, 0.0);
            double nonlinearity_1 = (NL_height * (2 * applySigmoid(total_input_dendrite_1, lambda, average_input_per_dendrite)) - (2 * s_at_zero)) + NL_center;
            ...
            // output dendritic currents get summed up
            ...
            { /* If cell is excitatory */
                ...
                if (refstate[j] <= 0)
                    v[j] += fmax((nonlinearity + gl[0] * DeltaT[0] * exp((v[j] - VT[0]) / DeltaT[0])) * dt / C[0], Vlb[0] - v[j]);
            ...
            { /* If cell is inhibitory */
                ...
                if (refstate[j] <= 0)
                    v[j] += fmax((nonlinearity - gl[1] * (v[j] - Vleak[1]) + gl[1] * DeltaT[1] * exp((v[j] - VT[1]) / DeltaT[1])) * dt / C[1], Vlb[1] - v[j]);
    ...
    if (ns == 0)
        mexWarnMsgTxt("No spikes fired during simulation.");
